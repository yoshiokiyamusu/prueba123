...
if($insert_uno === true){ //No continuar con el insert de otras tablas si no se guardo correctamente en tb.orden_de_servicio
  ...
  foreach ($skus as $vals) {
    ...
    if($insert_uno === true && $insert_dos === true && $insert_tres === true) {
      ...
      $url = 'view-orden_de_servicio/confirmar_orden_estampado.php?os=' . $os_post;
      //ChromePhp::log($url);
...
<?php
  ...
  $current_page = $_GET['page'] ?? 1;
      ...
      $per_page = 100;
      $total_count = orden_de_corte::oc_activas_count('ruta2'); //ChromePhp::log('total counti:  ' . $total_count);
      $pagination = new pagination($current_page, $per_page, $total_count);

      if($total_count < $per_page){ //cuanto hay menos records que el offset
        $records = orden_de_corte::oc_activas_sin_offset_page('ruta2',$per_page);
      }else{
        $num_rows_offset = $pagination->offset();
        $records = orden_de_corte::oc_activas_offset_page('ruta2',$per_page,$num_rows_offset);
      }
...
<section class="alternate page-heading">
  <div class="container">
   ...
   <form id="add-os" action="<?php echo url_for('view-orden_de_servicio/agregar_orden_estampado.php'); ?>" method="post">
     <h1>Agregar orden de estampado</h1>
...
<div class="row">
  <div id="index_tabla" class="col-md-12">
    ...
    <?php
           ...
           //pagination
           $url = url_for('view-orden_de_servicio/agregar_orden_estampado.php'); //
...
<!-- JS para la tabla dinamica con filtros -->
 ...
 <script src="<?php echo url_for('ajax/orden_de_servicio/unique_os_estampado.js'); ?>"></script>
